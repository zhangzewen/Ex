...
void dump_buffer(char *buff, int count)
{
	int i, j, c;
	int printnext = 1;
...
/* Compute a checksum */
u_short in_csum(u_short * addr, int len, u_short csum)
{
...
int inet_stosockaddr(char *ip, char *port, struct sockaddr_storage *addr)
{
	...
	if (addr->ss_family == AF_INET6) {
		...
		sscanf(ip, "%*[[]%[^]]", tmp);	// [aabb:ccdd::aacc] => aabb:ccdd::aacc
		ip = tmp;
		cp = ip;
	}
	...
	else
	{
		...
		addr_ip = &addr4->sin_addr;			/* point to sin_addr */
	}
...
char *inet_sockaddrtos2(struct sockaddr_storage *addr, char *addr_str)
{
	void *addr_ip;
...
int inet_sockaddrip6(struct sockaddr_storage *addr, struct in6_addr *ip6)
{
	if (addr->ss_family != AF_INET6)
		return -1;
