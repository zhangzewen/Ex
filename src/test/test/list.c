...
{
	element e = alloc_element();	/* allocate new element */

	e->prev = l->tail;
	e->data = data;			/* FIXME point to actual data */

	if (l->head == NULL)		/* list is empty */
		l->head = e;
	else
		l->tail->next = e;	/* add to list tail */
	l->tail = e;
	l->count++;			/* count */
}

void list_del(list l, void *data)
{
	element e;
...
static void free_element(list l)
{
	element e;
	element next;
...
void free_list_elements(list l)
{
	element e;
	element next;
...
void free_list_element(list l, element e)
{
	if (!e)
		return;
	if (l->head == e)
...
list alloc_mlist(void (*free_func) (void *), void (*dump_func) (void *), int size)
{
...
void dump_mlist(list l, int size)
{
	element e;
	int i;
...
void free_melement(list l, void (*free_func) (void *))
{
	element e;
	element next;
...
void free_mlist(list l, int size)
{
	int i;
