...
void xfree(void *p)
{
	mem_allocated -= sizeof(p);		/* kevin: bug */
	free(p);
	p = NULL;
}
...
char *keepalived_malloc(unsigned long size, char *file, char *function,
			int line)
{
	void *buf;
	int i = 0;
	...
	while (i < number_alloc_list)
	{
		if (alloc_list[i].type == 9 && alloc_list[i].ptr == buf)
		{
			...
			else
			{
				...
				if (debug & 1)
				{
					printf("free corrupt, buffer overrun [%3d:%3d], %p, %4ld at %s, %3d, %s\n", i, number_alloc_list, buf, alloc_list[i].size, file, line, function);
					dump_buffer(alloc_list[i].ptr,
						    alloc_list[i].size +
						    sizeof(long));
					printf("Check_sum\n");
					dump_buffer((char *) &alloc_list[i].
						    csum, sizeof(long));
...
void keepalived_free_final(char *banner)
{
	...
	while (i < number_alloc_list)
	{
		switch (alloc_list[i].type)
		{
			...
			case 4:
				badptr++;
				printf("pointer not found in table to free(%p) [%3d:%3d], at %s, %3d, %s\n", alloc_list[i].ptr, i, number_alloc_list, alloc_list[i].file, alloc_list[i].line, alloc_list[i].func);
				for (j = 0; j < 256; j++)
					if (free_list[j].ptr ==
					    alloc_list[i].ptr)
						if (free_list[j].type == 8)
							printf("  -> pointer allready released at [%3d:%3d], at %s, %3d, %s\n", (int) free_list[j].csum, number_alloc_list, free_list[j].file, free_list[j].line, free_list[j].func);
				break;
			case 2:
				badptr++;
				printf("null pointer to free(nil)! at %s, %3d, %s\n", alloc_list[i].file, alloc_list[i].line, alloc_list[i].func);
				break;
			case 1:
				overrun++;
...
void *keepalived_realloc(void *buffer, unsigned long size, char *file,
			 char *function, int line)
{
	...
	*(long *) ((char *) buf + size) = check;
	...
	if (debug & 1)
		printf("realloc [%3d:%3d] %p, %4ld %s %d %s -> %p %4ld %s %d %s\n", i, number_alloc_list, alloc_list[i].ptr, alloc_list[i].size, file, line, function, buf, size, alloc_list[i].file, alloc_list[i].line, alloc_list[i].func);
