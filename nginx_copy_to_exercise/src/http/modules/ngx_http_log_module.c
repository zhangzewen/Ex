...
/*
	...
	$msec 日子写入时间。单位为秒，精度是毫秒
	$pipe 如果请求是通过HTTP流水线(piplined)发送，pipe值为"p", 否则为"."
	$thrrp_referer 记录是从哪个页面连接访问过来的
	$http_user_agent 记录客户端浏览器相关信息
	$request_length 请求的长度(包括请求行，请求头和请求正文)
	$request_time 请求处理时间，单位为秒，精度毫秒从读入客户端的第一个字节开始，知道吧最后一个字符发送给客户端后进行日志写入为止
	$time_iso8601 ISO8601标准个试下的本地时间
