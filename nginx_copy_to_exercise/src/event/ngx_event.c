...
*/
    if (ngx_use_accept_mutex) {
				...
				如果ngx_accept_disabled大于0，就表示该进程接受的连接过多，因此就放弃一次争抢accept mutex的机会，同时将自己减1
				然后继续处理已有连接上的事件，Ng即使借用此变量实现了进程关于连接的基本负载均衡*/
        if (ngx_accept_disabled > 0) {
            ngx_accept_disabled--;

        } else {
/*
	尝试锁accept mutex，只有成功获取锁的进程，才能将listen套接字放入epoll中，因此，
	这就保证了只有一个进程拥有监听套接口，故所有、进程阻塞在epoll_wait事，不会出现惊群现象
...
/*
获取锁的进程，将添加一个NGX_POST_EVENTS标志，此标志的作用是将所产生的事件放入一个队列中，等释放锁后，
在慢慢处理事件。因为，处理事件可能会很耗时，如果不先释放锁再处理的话，该进程就长时间霸占了锁，
